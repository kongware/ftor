...
Please note that there are sometimes several type classes for a type and as a result of this alternative type representatives. However, since type reps are just plain old Javascript `Object`s it is easy to merge them, so that they meet the corresponding requirements.
